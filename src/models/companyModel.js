...
module.exports = {
  ...
  loginCompany: function (p_username) {
    //p_username is username that has been parse / 'oper'
    return new Promise(function (resolve, reject) {
      db.query(
  ...
  postCompany: function (body) {
    return new Promise(function (resolve, reject) {
      db.query(
        "insert into company set id_user=?, name=?, logo=?, location=?, description=?",
        [body.id_user, body.name, body.logo, body.location, body.description],
        function (err, response) {
          if (!err) {
            resolve(response);
          } else {
