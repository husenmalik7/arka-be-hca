...
module.exports = {
  getAllEngineer: (query) => {
    ...
    return new Promise((resolve, reject) => {
      db.query(
        `select * from engineer where id_company = 0 ${q_NameOrSkill} ${q_Sort} ${q_Limit}`,
  ...
  getProjectCompanyByIdEngineer: function (params) {
    return new Promise(function (resolve, reject) {
      db.query(
        "select * from engineer INNER join company on engineer.id_company = company.id_company where engineer.id_engineer = ?",
        [params],
        function (err, result) {
